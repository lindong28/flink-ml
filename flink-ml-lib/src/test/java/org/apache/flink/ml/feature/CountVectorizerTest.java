...
public class CountVectorizerTest extends AbstractTestBase {
    ...
    private static final List<SparseVectorWithIntIndex> EXPECTED_OUTPUT =
            new ArrayList<>(
                    Arrays.asList(
                            Vectors.sparse(
                                    6,
    ...
    private static void verifyPredictionResult(
            Table output, String outputCol, List<SparseVectorWithIntIndex> expected) throws Exception {
        StreamTableEnvironment tEnv =
                (StreamTableEnvironment) ((TableImpl) output).getTableEnvironment();
        DataStream<SparseVectorWithIntIndex> stream =
                tEnv.toDataStream(output)
                        .map(
                                (MapFunction<Row, SparseVectorWithIntIndex>)
                                        row -> (SparseVectorWithIntIndex) row.getField(outputCol));
        List<SparseVectorWithIntIndex> result = IteratorUtils.toList(stream.executeAndCollect());
    ...
    public void testMinMaxDF() throws Exception {
        List<SparseVectorWithIntIndex> expectedOutput =
                new ArrayList<>(
                        Arrays.asList(
                                Vectors.sparse(
                                        4,
